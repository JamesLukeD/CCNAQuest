...
| ---- | ---------- | ------------------------------------------------------------------------------------------ |
| ❤️   | **Hearts** | You have 5 lives. Lose one for every wrong answer. They refill one heart every 30 minutes. |
...
| H2  | **Hearts at 0**                         | `hearts === 0`                                                | Heart icons all empty (outlines). A banner beneath the HUD shows countdown: “❤️ Next heart in MM:SS”. Module cards are still tappable but lesson nodes show Alert blocking quiz launch.               | _(frog not shown)_                        | “❤️ Out of hearts. Hang tight, apprentice.” — live countdown replaces copy once `nextHeartAt` is set |
...
| Q2  | **Hearts at 0 on quiz start**           | `hearts === 0` when the user navigates to quiz                       | ✅ **Handled** — Alert fires blocking quiz launch. If user somehow enters quiz with 0 hearts, the ✕ escape button (top-left) is available on every question type to return to the section path.    | `incorrect.png`                       | Alert: “No hearts left — come back when you’ve refilled!”               |
...
**✅ Fixed — Bug #1 — Incremental heart refill (schema v6)**

Implemented 20 May 2026. `applyHeartRefill()` helper credits 1 heart per 30-minute interval since `nextHeartAt`. Called by `loadState()` on boot and by `checkHeartRefill()` on a 1-second interval in HomeScreen. `nextHeartAt` (Unix ms timestamp) persists in AsyncStorage as part of schema v6.

---

**✅ Fixed — Bug #2 — `resetHearts()` now restores to `MAX_HEARTS` (5)**

`resetHearts()` now sets `hearts: MAX_HEARTS` and clears `nextHeartAt: null`. The daily midnight reset (safety net in `loadState()`) also uses `MAX_HEARTS`.
