...
export default function ResultScreen() {
  const { lessonTitle, correct, wrong, total, streakUp, isReview, xpEarned } = useLocalSearchParams<{
    lessonTitle: string; correct: string; wrong: string; total: string; streakUp?: string; isReview?: string; xpEarned?: string;
  }>();
  ...
  const isFirstLesson = !isReviewLesson && !isTeachOnly && Object.keys(completed).length === 1;
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 24 }]}>
      ...
      <Animated.Text style={[styles.headline, {
        ...
        opacity: opacityAnim,
        color: isTeachOnly ? '#1cb0f6' : perfect ? '#58cc02' : good ? '#1cb0f6' : '#ff9600',
      }]}>
        {isTeachOnly
          ? '📖 Lesson complete!'
          : fizzled
            ? '💀 The spell fizzled.'
            : perfect ? '🎉 Perfect!' : good ? '👍 Nice Work!' : '💪 Keep Going!'}
      </Animated.Text>
      {fizzled && (
      ...
      <View style={styles.actions}>
        ...
        <PrimaryButton label="← Try Again" onPress={() => router.back()} variant="secondary" />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  streakBanner: {
    ...
    backgroundColor: '#2a1500', borderWidth: 1, borderColor: '#ff9600aa',
  ...
  firstLessonBanner: {
    alignItems: 'center',
    backgroundColor: '#1a1000', borderWidth: 1, borderColor: '#ffd700aa',
